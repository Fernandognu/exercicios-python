#### Exercício 28

#### O Hipermercado Tabajara está com uma promoção de carnes que é imperdível. Confira:
#### ```
####                 Até 5 Kg                Acima de 5 Kg
#### File Duplo      R$ 4,90 por Kg          R$ 5,80 por Kg
#### Alcatra         R$ 5,90 por Kg          R$ 6,80 por Kg
#### Picanha         R$ 6,90 por Kg          R$ 7,80 por Kg
#### ```
#### Para atender a todos os clientes, cada cliente poderá levar apenas um dos tipos de carne da promoção, porém não há limites para a quantidade de carne por cliente. Se compra for feita no cartão Tabajara o cliente receberá ainda um desconto de 5% sobre o total da compra. Escreva um programa que peça o tipo e a quantidade de carne comprada pelo usuário e gere um cupom fiscal, contendo as informações da compra: tipo e quantidade de carne, preço total, tipo de pagamento, valor do desconto e valor a pagar.

tipo_carne = input("Digite o tipo de carne (File Duplo, Alcatra, Picanha): ").strip().lower()
quantidade_kg = float(input("Digite a quantidade de carne em Kg: "))
preco_por_kg = 0
if tipo_carne == "file duplo":
    if quantidade_kg <= 5:
        preco_por_kg = 4.90
    else:
        preco_por_kg = 5.80
elif tipo_carne == "alcatra":
    if quantidade_kg <= 5:
        preco_por_kg = 5.90
    else:
        preco_por_kg = 6.80
elif tipo_carne == "picanha":
    if quantidade_kg <= 5:
        preco_por_kg = 6.90
    else:
        preco_por_kg = 7.80
else:
    print("Tipo de carne inválido.")
    exit()
preco_total = preco_por_kg * quantidade_kg
tipo_pagamento = input("Digite o tipo de pagamento (Cartão Tabajara ou Outro): ").strip().lower()
desconto = 0
if tipo_pagamento == "cartão tabajara":
    desconto = preco_total * 0.05
valor_a_pagar = preco_total - desconto
print("\n--- Cupom Fiscal ---")
print(f"Tipo de carne: {tipo_carne.title()}")
print(f"Quantidade de carne: {quantidade_kg:.2f} Kg")
print(f"Preço total: R$ {preco_total:.2f}")
print(f"Tipo de pagamento: {tipo_pagamento.title()}")
print(f"Valor do desconto: R$ {desconto:.2f}")
print(f"Valor a pagar: R$ {valor_a_pagar:.2f}")